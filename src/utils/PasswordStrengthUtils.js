...
function checkPasswordStrength(password) {
  ...
  const crackTimeOnlineNoThrottling =
    crackTimes.online_no_throttling_10_per_second;
  const crackTimeOfflineSlowHashing =
    crackTimes.offline_slow_hashing_1e4_per_second;
  const crackTimeOfflineFastHashing =
  ...
  if (score === 4) {
    ...
    if (
      crackTimeOnlineThrottling === "centuries" &&
      crackTimeOnlineNoThrottling === "centuries" &&
      crackTimeOfflineSlowHashing === "centuries" &&
      crackTimeOfflineFastHashing === "centuries" &&
      zxcvbnResult.feedback.warning.length === 0 &&
      minGuess.guesses >= cryptoConfig.passwordGuessThreshold
    ) {
